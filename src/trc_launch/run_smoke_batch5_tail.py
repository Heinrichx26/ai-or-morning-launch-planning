...
def main() -> None:
    output_dir = Path("results/trc_smoke/batch5_tail")
    data_dir = Path("data/trc_open_smoke/batch5_tail")
    output_dir.mkdir(parents=True, exist_ok=True)
    data_dir.mkdir(parents=True, exist_ok=True)

    month = load_bts_month(DEFAULT_BTS_CSV)
    instances = {}
    for carrier, date in TAIL_CASES:
        ...
        inst.flights.to_csv(data_dir / f"flights_tail_{carrier}_{date}.csv", index=False)
        inst.inventory.to_csv(data_dir / f"inventory_tail_{carrier}_{date}.csv", index=False)
        inst.candidates.to_csv(data_dir / f"candidates_tail_{carrier}_{date}.csv", index=False)
        inst.scenarios.to_csv(data_dir / f"scenarios_tail_{carrier}_{date}.csv", index=False)
    ...
    for key, instance in instances.items():
        ...
        teacher_results[key] = LaunchOptimizer(instance, time_limit=25.0).solve(
            "regret_portfolio",
    ...
    for test_key, instance in instances.items():
        ...
        if train_frames:
            ...
            for repair in [False, True]:
                ...
                result.selected.to_csv(
                    output_dir / f"selected_tail_{result.method}_{test_key}.csv",
                    index=False,
                )
        ...
        for method in methods:
            if method == "regret_portfolio":
                ...
                result = LaunchOptimizer(instance, time_limit=25.0).solve(method)
            metrics = evaluate_solution(instance, result, oracle_costs=oracle_costs_by_key[test_key])
            metrics["instance_key"] = test_key
            rows.append(metrics)
            if not result.selected.empty:
                result.selected.to_csv(output_dir / f"selected_tail_{method}_{test_key}.csv", index=False)

    metrics = pd.DataFrame(rows)
    metrics.to_csv(output_dir / "batch5_tail_metrics.csv", index=False)
    summary = (
    ...
    summary.to_csv(output_dir / "batch5_tail_summary.csv", index=False)
